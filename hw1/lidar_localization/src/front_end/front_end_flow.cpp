...
bool FrontEndFlow::Run() {
  ...
  while (HasData()) {
    LOG(INFO) << "[Run] data parsed";
    if (!ValidData()) continue;

    LOG(INFO) << "[Run] data valid";
    UpdateGNSSOdometry();
    if (UpdateLaserOdometry()) {
      LOG(INFO) << "[Run] laser odom updated";
      PublishData();
      SaveTrajectory();
    } else {
      LOG(INFO) << "[Run] UpdateLaserOdometry failed !!!";
    }
  }
...
bool FrontEndFlow::ReadData() {
  ...
  if (cloud_data_buff_.size() == 0) {
    // LOG(INFO) << "no cloud data";
    return false;
  }

  double cloud_time = cloud_data_buff_.front().time;
  LOG(INFO) << "[ReadData] cloud front timestamp, " << std::to_string(cloud_time).substr(0, 14);
  ...
  if (!sensor_inited) {
    if (valid_imu == false || valid_velocity == false || valid_gnss == false) {
      LOG(INFO) << "[ReadData] no valid imu or vel or gnss data, " << valid_imu << " " << valid_velocity << " " << valid_gnss;
      cloud_data_buff_.pop_front();
      return false;
    }
    LOG(INFO) << "[ReadData] all data valid, sensor init";
    sensor_inited = true;
  }
...
bool FrontEndFlow::SaveTrajectory() {
  ...
  if (!is_file_created) {
    if (!FileManager::CreateDirectory(WORK_SPACE_PATH + "/slam_data/trajectory")) return false;
    if (!FileManager::CreateFile(ground_truth, WORK_SPACE_PATH + "/slam_data/trajectory/ground_truth.txt"))
      return false;
    if (!FileManager::CreateFile(laser_odom, WORK_SPACE_PATH + "/slam_data/trajectory/laser_odom.txt")) return false;

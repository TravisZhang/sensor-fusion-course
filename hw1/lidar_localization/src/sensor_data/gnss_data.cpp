...
//静态成员变量必须在类外初始化
bool lidar_localization::GNSSData::origin_position_inited = 0;
...
bool GNSSData::SyncData(std::deque<GNSSData>& UnsyncedData, std::deque<GNSSData>& SyncedData, double sync_time) {
  ...
  LOG(INFO) << "[SyncData] gnss unsync data size, " << UnsyncedData.size();
  ...
  GNSSData back_data = UnsyncedData.back();
  if (UnsyncedData.size() > 1) {
    back_data = UnsyncedData.at(1);
  }
  GNSSData synced_data;

  // LOG(INFO) << "[SyncData] gnss front time, " << std::to_string(front_data.time).substr(0, 14) << " back time, "
